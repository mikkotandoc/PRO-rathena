# NPCs & Merchants

[← Back to Home](Home.md)

Directory of **enabled** custom NPCs from `npc/scripts_custom.conf` on **PRO-Server-Live**.

---

## Prontera — Core Services

| NPC | Coords | Service |
|-----|--------|---------|
| Episode Guide | 165, 175 | Warp to episode hubs (17–20) |
| Instance & EP Merchant | 164, 172 | Illusion & episode barter shops |
| Centro (Booster) | 166, 300 | Booster Coin exchanges |
| Job Master | 153, 193 | Job changes through 4th class |
| Silvervine Quester | 153, 191 | 10 daily mini-boss hunts |
| Reset Girl | 150, 193 | Stat/skill reset (5k / 5k / 9k zeny) |
| Healer | 162, 193 | **Free** heal, buffs, identify all |
| Warper | 159, 192 | Towns, fields, dungeons, instances, Item Mall |
| Platinum Skill NPC | 128, 200 | Quest / platinum skills |
| Stylist | 170, 180 | Hair & clothing |
| Breeder (Rental) | 124, 201 | Cart, Peco, Falcon, Mado rentals |
| Bank Clerk (Kafra) | 131, 190 | Account bank |
| King Poring | 174, 176 | Card recycler (4 cards + 2.5M z) |
| Plagiarism Master | 142, 172 | Rogue skill copy (25k z) |
| Earl (Cash Coin Shop) | 144, 178 | Playtime points + PRO-Asia coin shops |
| Private MVP Room | 148, 174 | Rent MVP room (100k z / 60 min) |
| Barter NPC | *(see barter_npc)* | Same as Instance Merchant |
| GM Episode Debug | 167, 173 | Staff episode testing |

---

## Item Mall (`prt_mk`)

Warp: **Warper → Item Mall** or menu entry to `prt_mk 100,95`

| NPC | Coords | Service |
|-----|--------|---------|
| Equipment Merchant | 114, 98 | Basic weapons & armor |
| Headgear & Garment Dealer | 114, 101 | Headgear, garments |
| Supply Dealer | 114, 104 | Potions & consumables |
| Catalyst Supplier | 114, 107 | Alchemy catalysts |

---

## Prontera Castle

| NPC | Coords | Service |
|-----|--------|---------|
| Balheele | 373, 77 | Episode Clear tickets (Silvervine) |

---

## Prontera Inn

| NPC | Coords | Service |
|-----|--------|---------|
| Wise Old Woman | prt_in 28, 73 | Card removal (200k z + materials) |

---

## Lighthalzen

| NPC | Coords | Service |
|-----|--------|---------|
| PVP Ladder Registrar | 154, 106 | PVP tournament — 18:00 & 21:00 |

---

## Comodo

| NPC | Coords | Service |
|-----|--------|---------|
| Phantom's Spirit | 214, 186 / 208, 187 | Corridor of Phantoms passes |

---

## Eden Group HQ (`moc_para01`)

| Service | NPC / Area |
|---------|------------|
| Registration | Secretary Lime Evenor (27, 35) |
| Beginner training | Instructor Boya (25, 35) |
| Advanced training | Instructor Ur (23, 35) |
| MVP exchange | Eve Natalia area (53, 33) |
| Eden events | Daily hunt board |
| EAT gear mimic | Eden HQ script |

---

## Episode Hubs (via Episode Guide)

### Episode 17.1 — Illusion / Cor

| Map | Content |
|-----|---------|
| `sp_cor` | Cor Operation — Grace gear, RS26 enchants |
| `sp_rudus` | Rudus facility |
| `sp_os` | OS occupation area |
| `lighthalzen` | Main 17.1 story hub |

### Episode 17.2 — Sage's Legacy

| Map | Content |
|-----|---------|
| `ba_in01` | Baggot Lab merchants & enchants |
| `ba_pw01` | Pax Employment |
| `ba_in01` | Biosphere Depth Researcher (BL 250+) |

### Episode 18 — Wolf Village

Hub: `wolfvill` — storyline merchants & instances

### Episode 19 — Ice Castle

Hub: `icas_in` — Ice Castle traders

### Episode 20 — Yggdrasil

Hub: `jor_nest` — endgame episode content

---

## Malangdo (Costumes)

Official kRO costume exchange & enchant NPCs (`malangdo_costume.txt`):

| NPC | Role |
|-----|------|
| Gregio Grumani | 4th-slot visual enchant |
| Aver De Dosh | Costume box exchanges |
| Yves | Booster promotional costume enchant |

---

## Instance & Episode Barters

**NPC:** Instance & EP Merchant — `prontera 164,172`

**Illusion shops:** Vampire, Underwater, Twins, Moonlight, Frozen, Turtle, Luanda, Teddy Bear, Resonance Stone

**Episode shops:** Grace equipment by job, rebellion supplies, automatic gear, teleport tickets, reinforcement cubes (Ep 14–20)

---

## Quest NPCs (Enabled)

| Quest | Theme |
|-------|-------|
| Elven Ears | Classic headgear |
| Sunglasses | Classic headgear |
| Bandit Beard | Classic headgear |
| May Hats / Event 6 Hats | Custom headgear |
| King's Items | Custom headgear |
| Valhallen | Godly equipment |
| Kaho Horn | High-end headgear |

Locations vary — use `@whereis` on quest items or ask on Discord.

---

## Zeny Price Reference

| Service | Cost |
|---------|------|
| Healer | **Free** |
| Reset skills | 5,000 z |
| Reset stats | 5,000 z |
| Reset both | 9,000 z |
| Plagiarism | 25,000 z |
| Card remover | 200,000 z + 25,000 z/card + materials |
| King Poring recycle | 2,500,000 z |
| MVP private room | 100,000 z / hour |
| PVP Ladder entry | 50,000 z |

---

## Disabled NPCs (not loaded)

Card Seller, WOE Controller, MVP Arena, Lottery, Stock Market, Hunting Missions, Quest Shop, most holiday events, Battleground recruiters, Floating Rates, Costume Recycler.

---

## See Also

- [Custom Systems](Custom-Systems.md)
- [Items & Equipment](Items-and-Equipment.md)
- [Home](Home.md)
