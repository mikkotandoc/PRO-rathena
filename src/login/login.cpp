...
void login_print_console_status(void) {
	...
	ShowStatus( "  Shield handshake: %s (0x0af9 challenge / 0x0af4 response)\n", login_config.shield_handshake_check ? "enabled" : "disabled" );
...
bool LoginServer::initialize( int32 argc, char* argv[] ){
	...
	ShowStatus("PRO Anti-Cheat: shield_handshake_check is %s, map-compatible 0x0af9/0x0af5 (console: server:status)\n", login_config.shield_handshake_check ? CL_GREEN "enabled" CL_RESET : CL_YELLOW "disabled" CL_RESET);
